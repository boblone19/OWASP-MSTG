...
- **code_cache**: 这是文件系统的应用程序特定缓存目录的位置，该目录旨在存储缓存代码。在运行 Android 5.0（API 级别 21）或更高版本的设备上，当应用或整个平台升级时，系统将删除存储在此位置的任何文件。
- **lib**: 此文件夹存储以 C/C++编写的本机库。这些库可以具有多个文件扩展名之一，包括 .so 和 .dll（x86 支持）。此文件夹包含应用具有本机库的平台的子目录，包括
  ...
  - x86: 仅为 x86 处理器编译代码
  - x86_64: 仅为x86_64处理器编译的代码
  - mips: MIPS处理器的编译代码
- **shared_prefs**: 此文件夹包含一个 XML 文件，该文件存储通过 保存的值 [共享首选项 API](https://developer.android.com/training/basics/data-storage/shared-preferences.html "SharedPreferences APIs").
...
在 Android 上，您可以使用 [`Logcat`](https://developer.android.com/tools/debugging/debugging-log.html "Debugging with Logcat")轻松检查系统消息的日志。有两种执行 Logcat 的方法：
...
[可以使用tcpdump，netcat（nc）和Wireshark实时远程嗅探所有Android流量](https://blog.dornea.nu/2015/02/20/android-remote-sniffing-using-tcpdump-nc-and-wireshark/ "Android remote sniffing using Tcpdump, nc and Wireshark"). 首先，请确保手机上具有[Android tcpdump](https://www.androidtcpdump.com/) 的最新版本。 这是[安装步骤](https://wladimir-tm4pda.github.io/porting/tcpdump.html "Installing tcpdump"):
...
FCM适用于Android，iOS和Chrome。 FCM当前提供两种连接服务器协议：HTTP和XMPP。 如[官方文档](https://firebase.google.com/docs/cloud-messaging/server#choose "Differences of HTTP and XMPP in FCM"), 中所述，这些协议的实现方式有所不同。 下面的示例演示如何拦截这两种协议。
...
对于XMPP通信，[FCM使用端口](https://firebase.google.com/docs/cloud-messaging/xmpp-server-ref "Firebase via XMPP") 5235（生产）和5236（测试）。
...
作为附加的安全层，可以使用[Capillary](https://github.com/google/capillary "Capillary")对推送通知进行加密。 Capillary是一个库，用于简化从基于Java的应用程序服务器到Android客户端的端到端（E2E）加密推送消息的发送。
...
启动AVD时，也可以使用[emulator command](https://developer.android.com/studio/run/emulator-commandline "Emulator Command") 在命令行上配置AVD代理。 以下示例启动AVD Nexus_5X_API_23，并将代理设置为127.0.0.1和端口8080。
...
对于Android 7.0（API级别24）及更高版本，请遵循“ [绕过网络安全配置](#bypassing-the-network-security-configuration "Bypassing the Network Security Configuration")" 部分中所述的相同步骤。

##### 物理设备的拦截代理

必须首先评估可用的网络设置选项。 用于测试的移动设备和运行拦截代理的计算机必须连接到同一Wi-Fi网络。 使用（现有的）接入点或创建[临时无线网络](https://support.portswigger.net/customer/portal/articles/1841150-Mobile%20Set-up_Ad-hoc%20network_OSX.html "Creating an Ad-hoc Wireless Network in OS X").

配置网络并在测试机和移动设备之间建立连接后，需要执行几个步骤

- 必须将代理[配置为指向拦截代理](https://support.portswigger.net/customer/portal/articles/1841101-Mobile%20Set-up_Android%20Device.html "Configuring an Android Device to Work With Burp").
- [拦截代理的CA证书必须添加到Android设备的证书存储中的受信任证书中](https://support.portswigger.net/customer/portal/articles/1841102-installing-burp-s-ca-certificate-in-an-android-device "Installing Burp's CA Certificate in an Android Device").在一个Android设备中“在Android设备中安装Burp的CA证书”。用于存储CA证书的菜单的位置可能取决于设置菜单的Android版本和Android OEM修改。
- 某些应用程序（例如[Chrome浏览器](https://bugs.chromium.org/p/chromium/issues/detail?id=475745 "Chromium Issue 475745")) 可能会显示 `NET::ERR_CERT_VALIDITY_TOO_LONG` 错误，如果树叶证书的有效期延长了一定时间（Chrome为39个月）。如果使用默认的Burp CA证书，则会发生这种情况，因为Burp Suite发行的叶证书的有效性与其CA证书相同。您可以通过创建自己的CA证书并将其导入Burp Suite来规避此问题，如[nviso.be上的博客文章](https://blog.nviso.be/2018/01/31/using-a-custom-root-ca-with-burp-for-inspecting-android-n-traffic/ "Using a custom root CA with Burp for inspecting Android N traffic").

完成这些步骤并启动应用程序后，请求应显示在拦截代理中。

> 可以找到使用Android设备设置OWASP ZAP的视频[secure.force.com](https://security.secure.force.com/security/tools/webapp/zapandroidsetup "Setting up ZAP for Android").
...
为了实现应用程序的网络安全配置，您需要创建一个名为`network_security_config.xml`的新xml资源文件。在[Google Android Codelabs](https://codelabs.developers.google.com/codelabs/android-network-security-config/#3 "Basic Network Security Configuration").
...
网络安全配置使用XML文件，其中应用程序指定将信任哪些CA证书。有多种绕过网络安全配置的方法，下面将进行描述。另请参阅[Android P中网络安全配置的安全分析师指南](https://www.nowsecure.com/blog/2018/08/15/a-security-analysts-guide-to-network-security-configuration-in-android-p/ "Security Analyst’s Guide to Network Security Configuration in Android P") 以获取更多信息

###### 将用户证书添加到网络安全配置

网络安全配置有不同的配置可通过src属性[添加非系统证书颁发机构](https://developer.android.com/training/articles/security-config#CustomTrust "Custom Trust") :
...
- 您需要按照“反向工程和篡改”一章的“ [重新打包](0x05c-Reverse-Engineering-and-Tampering.md#repackaging "Repackaging")" 部分中的说明对应用程序进行重新打包。有关重新打包过程的更多详细信息，您还可以查阅[Android开发者文档](https://developer.android.com/studio/publish/app-signing#signing-manually), 该文件从整体上说明了该过程。
...
有一个Python脚本可以自动执行上述称为 [Android-CertKiller](https://github.com/51j0/Android-CertKiller "Android-CertKiller").的步骤。该Python脚本可以从已安装的Android应用中提取APK，对其进行反编译，使其可调试，添加新的网络安全配置，该配置允许用户证书，构建和签名新APK以及使用SSL旁路安装新APK。最后一步，[安装应用程序可能失败](https://github.com/51j0/Android-CertKiller/issues "APK not installing"), 由于当前存在错误。
...
有一个[Magisk模块](https://github.com/NVISO-BE/MagiskTrustUserCerts "Magisk Trust User Certs") 它将自动将所有用户安装的CA证书添加到系统信任的CA列表中。

下载模块的最新版本[此处](https://github.com/NVISO-BE/MagiskTrustUserCerts/releases "Magisk Trust User Certs - Releases"),BE/MagiskTrustUserCerts/releases“ Magisk信任用户证书-发布”），将下载的文件推送到设备上，并将其导入到Magisk中通过单击“ +”按钮，管理器的“模块”视图。最后，Magisk Manager需要重新启动才能使更改生效。
...
-使/ system分区可写，这只能在有根设备上进行。 运行'mount'命令以确保/ system是可写的：`mount -o rw，remount / system`。 如果此命令失败，请尝试运行以下命令“ mount -o rw，remount -t ext4 / system”
-准备代理的CA证书以匹配系统证书格式。 以“ der”格式（这是Burp Suite的默认格式）导出代理的证书，然后运行以下命令：
...
[无线客户端隔离](https://documentation.meraki.com/MR/Firewall_and_Traffic_Shaping/Wireless_Client_Isolation "Wireless Client Isolation") 是一项安全功能，可防止无线客户端相互通信。 此功能对来宾和BYOD SSID有用，它增加了安全级别，以限制连接到无线网络的设备之间的攻击和威胁。
...
为了绕过这种保护机制，您可以设置Bettercap或配置不需要在Android手机上设置代理的iptables。我们之前没有提到的第三个选项是使用Frida，它在这种情况下适用。在Android上，可以通过查询[`ProxyInfo`](https://developer.android.com/reference/android/net/ProxyInfo "ProxyInfo") 类并检查getHost（）来检测是否设置了系统代理和getPort（）方法。可能会有多种其他方法来完成相同的任务，因此您需要对APK进行反编译以标识实际的类和方法名称。
