...
* [otool](http://www.manpagez.com/man/1/otool/) is a tool for displaying specific parts of object files or libraries. It works with Mach-O files and universal file formats.
* [nm](http://www.manpagez.com/man/1/nm/osx-10.12.6.php) is a tool that displays the name list \(symbol table\) of the given binary.
...
* [Ghidra](https://ghidra-sre.org/) is a software reverse engineering \(SRE\) suite of tools developed by NSA's Research Directorate. This tool has been discussed in "[Ghidra](https://github.com/boblone19/owasp-mstg/tree/157b0709348524cbba3620c6dcd1a1162ffa6bb6/Document/0x04c-tampering-and-reverse-engineering.md#ghidra)" section.

#### Building a Reverse Engineering Environment for Free

Be sure to follow the instructions from the section "Setting up Xcode and Command Line Tools" of chapter "iOS Basic Security Testing". This way you'll have properly installed [Xcode](https://developer.apple.com/xcode/ide/). We'll be using standard tools that come with macOS and Xcode in addition to the tools mentioned above. Make sure you have the [Xcode command line developer tools](https://railsapps.github.io/xcode-command-line-tools.html) properly installed or install them straight away from your terminal:

```text
$ xcode-select --install
```

* [`xcrun`](http://www.manpagez.com/man/1/xcrun/) can be used invoke Xcode developer tools from the command-line, without having them in the path. For example you may want to use it to locate and run swift-demangle or simctl.
...
* [IDA Pro](https://www.hex-rays.com/products/ida/) can deal with iOS binaries. It has a built-in iOS debugger. IDA is widely seen as the gold standard for GUI-based interactive static analysis, but it isn't cheap. For the more budget-minded reverse engineer, [Hopper](https://www.hopperapp.com/) offers similar static analysis features.
* [Hopper](https://www.hopperapp.com/) is a reverse engineering tool for macOS and Linux used to disassemble, decompile and debug 32/64bits Intel Mac, Linux, Windows and iOS executables.

### Disassembling and Decompiling

Because Objective-C and Swift are fundamentally different, the programming language in which the app is written affects the possibilities for reverse engineering it. For example, Objective-C allows method invocations to be changed at runtime. This makes hooking into other app functions \(a technique heavily used by [Cycript](http://www.cycript.org/) and other reverse engineering tools\) easy. This "method swizzling" is not implemented the same way in Swift, and the difference makes the technique harder to execute with Swift than with Objective-C.
...
![IDA Pro open a Mach-O file](../.gitbook/assets/ida_macho_import.png)
...
![IDA Pro main window](../.gitbook/assets/ida_main_window.png)
...
In addition to the techniques learned in the "[Disassembling and Decompiling](0x06c-reverse-engineering-and-tampering.md#disassembling-and-decompiling)" section, for this section you'll need some understanding of the [Objective-C runtime](https://developer.apple.com/documentation/objectivec/objective-c_runtime). For instance, functions like `_objc_msgSend` or `_objc_release` are specially meaningful for the Objective-C runtime.

We will be using the [UnCrackable Level 1 crackme app](https://github.com/OWASP/owasp-mstg/blob/master/Crackmes/iOS/Level_01/UnCrackable_Level1.ipa), which has the simple goal of finding a _secret string_ hidden somewhere in the binary. The application has a single home screen and a user can interact via inputting custom strings in the provided text field.

![Home screen of the UnCrackable Level 1 application](../.gitbook/assets/manual_reversing_app_home_screen.png)

When the user inputs the wrong string, the application shows a pop-up with the "Verification Failed" message.

![Verification Failed Pop-Up](../.gitbook/assets/manual_reversing_app_wrong_input.png)
...
> For static analysis in this section, we will be using Ghidra 9.0.4. Ghidra 9.1\_beta auto-analysis has a bug and does not show the Objective-C classes.
...
![Objective-C runtime related strings](../.gitbook/assets/manual_reversing_ghidra_objc_runtime_strings.png)
...
![Interesting strings in the UnCrackable Level 1 application](../.gitbook/assets/manual_reversing_ghidra_strings.png)
...
An iOS application calls different predefined functions provided by the iOS runtime depending on its the state within the \[application life cycle\]\([https://developer.apple.com/documentation/uikit/app\_and\_environment/managing\_your\_app\_s\_life\_cycle](https://developer.apple.com/documentation/uikit/app_and_environment/managing_your_app_s_life_cycle) "Managing Your App's Life Cycle"\). These functions are known as the entry points of the app. For example:
...
Luckily the current application has a small code base, and we can find another `ViewController` class in the **Symbol Tree** view. In this class, function `viewDidLoad` function looks interesting. If you check the documentation of [`viewDidLoad`](https://developer.apple.com/documentation/uikit/uiviewcontroller/1621495-viewdidload), you can see that it can also be used to perform additional initialization on views.

![Decompilation of viewDidLoad function](../.gitbook/assets/manual_reversing_ghidra_viewdidload_decompile.png)
...
![Disassembly of the native function](../.gitbook/assets/manual_reversing_ghidra_native_disassembly.png)
...
[Objection](https://github.com/sensepost/objection) is a mobile runtime exploration toolkit based on Frida. One of the biggest advantages about Objection is that it enables testing with non-jailbroken devices. It does this by automating the process of app repackaging with the `FridaGadget.dylib` library. A detailed explanation of the repackaging and resigning process can be found in the next chapter "Manual Repackaging". We won't cover Objection in detail in this guide, as you can find exhaustive documentation on the official [wiki pages](https://github.com/sensepost/objection/wiki).
...
To reproduce the steps listed below, download [UnCrackable iOS App Level 1](https://github.com/OWASP/owasp-mstg/tree/master/Crackmes/iOS/Level_01) from the OWASP Mobile Testing Guide repository. Our goal is to make the UnCrackable app load `FridaGadget.dylib` during startup so we can instrument the app with Frida.

> Please note that the following steps apply to macOS only, as Xcode is only available for macOS.

#### Getting a Developer Provisioning Profile and Certificate

The _provisioning profile_ is a plist file signed by Apple. It whitelists your code-signing certificate on one or more devices. In other words, this represents Apple explicitly allowing your app to run for certain reasons, such as debugging on selected devices \(development profile\). The provisioning profile also includes the _entitlements_ granted to your app. The _certificate_ contains the private key you'll use to sign.
...
If you've developed and deployed iOS apps with Xcode before, you already have your own code-signing certificate installed. Use the _security_ tool to list your signing identities:
...
In the following examples, we'll assume that you are working on a jailbroken device. If that's not the case, you first need to follow the steps outlined in section [Repackaging and Re-Signing](0x06c-reverse-engineering-and-tampering.md#repackaging-and-re-signing) to repackage the Safari app.
...
An introduction to binary analysis using binary analysis frameworks has already been discussed in the "[Dynamic Analysis](https://github.com/boblone19/owasp-mstg/tree/157b0709348524cbba3620c6dcd1a1162ffa6bb6/Document/0x05c-reverse-engineering-and-tampering/README.md#dynamic-analysis)" section for Android. We recommend you to revisit this section and refresh the concepts on this subject.
...
While manual analyzing the code in the [Reviewing Disassembled Native Code](0x06c-reverse-engineering-and-tampering.md#reviewing-disassembled-native-code)" section, we reached a point where performing further manual analysis was cumbersome. The function at offset `0x1000080d4` was identified as the final target which contains the secret string.
...
Finally, we use the codesign tool to re-sign both binaries. You need to use _your_ signing identity \(in this example 8004380F331DCA22CC1B47FB1A805890AE41C938\), which you can output by executing the command `security find-identity -v`.
...
![Frida on non-JB device](../.gitbook/assets/fridastockios.png)

When something goes wrong \(and it usually does\), mismatches between the provisioning profile and code-signing header are the most likely causes. Reading the [official documentation](https://developer.apple.com/support/code-signing/) helps you understand the code-signing process. Apple's [entitlement troubleshooting page](https://developer.apple.com/library/content/technotes/tn2415/_index.html) is also a useful resource.
...
Start Cydia and add Frida's repository by navigating to **Manage** -&gt; **Sources** -&gt; **Edit** -&gt; **Add** and entering [https://build.frida.re](https://build.frida.re). You should then be able to find and install the Frida package.
...
![Cycript Alert Sample](../.gitbook/assets/cycript_sample.png)
...
You can dump the app's process memory with [objection](https://github.com/sensepost/objection) and [Fridump](https://github.com/Nightbringer21/fridump). To take advantage of these tools on a non-jailbroken device, the Android app must be repackaged with `frida-gadget.so` and re-signed. A detailed explanation of this process is in the section "[Dynamic Analysis on Non-Jailbroken Devices](0x06c-reverse-engineering-and-tampering.md#dynamic-analysis-on-non-jailbroken-devices). To use these tools on a jailbroken phone, simply have frida-server installed and running.
...
In both cases, if you open the file in radare2 you can use its search command \(`/`\). Note that first we do a standard string search which doesn't succeed and next we search for a [wide string](https://en.wikipedia.org/wiki/Wide_character), which successfully finds our string "owasp-mstg".
...
* Class-dump-dyld - [https://github.com/limneos/classdump-dyld/](https://github.com/limneos/classdump-dyld/)
* Class-dump-z - [https://code.google.com/archive/p/networkpx/wikis/class\_dump\_z.wiki](https://code.google.com/archive/p/networkpx/wikis/class_dump_z.wiki)
* Cycript - [http://www.cycript.org/](http://www.cycript.org/)
* Damn Vulnerable iOS App - [http://damnvulnerableiosapp.com/](http://damnvulnerableiosapp.com/)
...
* ipainstaller - [https://cydia.saurik.com/package/com.slugrail.ipainstaller/](https://cydia.saurik.com/package/com.slugrail.ipainstaller/)
* MachoOView - [https://sourceforge.net/projects/machoview/](https://sourceforge.net/projects/machoview/)
* Objection - [https://github.com/sensepost/objection](https://github.com/sensepost/objection)
* Optool - [https://github.com/alexzielenski/optool](https://github.com/alexzielenski/optool)
* OWASP UnCrackable Apps for iOS - [https://github.com/OWASP/owasp-mstg/tree/master/Crackmes\#ios](https://github.com/OWASP/owasp-mstg/tree/master/Crackmes#ios)
* r2frida - [https://github.com/nowsecure/r2frida](https://github.com/nowsecure/r2frida)
* Radare2 - [https://rada.re/r/](https://rada.re/r/)
* Reverse Engineering tools for iOS Apps - [http://iphonedevwiki.net/index.php/Reverse\_Engineering\_Tools](http://iphonedevwiki.net/index.php/Reverse_Engineering_Tools)
* Swizzler project - [https://github.com/vtky/Swizzler2/](https://github.com/vtky/Swizzler2/)
* Xcode command line developer tools - [https://railsapps.github.io/xcode-command-line-tools.html](https://railsapps.github.io/xcode-command-line-tools.html)
